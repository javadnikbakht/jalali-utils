...
import pytz
from khayyam import TehranTimezone, JalaliDatetime

from django_toman_time_utils.functions import convert_date_to_int, convert_time_to_int
from django_toman_time_utils.timezone import TEHRAN_ZONE, TehranTimezone
...
def get_today_tehran_jalali_date_strftime(string_format="%Y/%m/%d") -> str:
    return get_now_tehran_jalali_datetime().date().strftime(string_format)


def get_today_tehran_jalali_time_strftime(string_format="%H:%M:%S") -> str:
    return get_now_tehran_jalali_datetime().time().strftime(string_format)


def get_today_tehran_jalali_date_intftime() -> str:
    """
    convert today Jalali date as a integer date
    e.g: datetime(2023-03-29) => 1402/01/09 => 14020109
    """
    today_jalali_date_strftime = get_today_tehran_jalali_date_strftime(string_format="%Y-%m-%d")
    return convert_date_to_int(today_jalali_date_strftime)


def get_today_tehran_jalali_time_intftime() -> str:
    today_jalali_time_strftime = get_today_tehran_jalali_time_strftime(string_format="%H:%M:%S")
